...
const client = new CasperClient("http://65.109.145.211:7777")
const contract = new Contracts.Contract(client)



async function install_contract(){

    const runtime_args = RuntimeArgs.fromMap({
        collection_name: CLValueBuilder.string("Richemont"),
        collection_symbol: CLValueBuilder.string("RCM"),
    ...
    const client = new CasperClient("http://65.109.145.211:7777");
    const contract = new Contracts.Contract(client);
    const contractWasm = new Uint8Array(fs.readFileSync("./cep-78-enhanced-nft/contract/target/wasm32-unknown-unknown/release/contract.wasm").buffer);
    const runtimeArguments = runtime_args;
    const keypair = Keys.Ed25519.loadKeyPairFromPrivateFile("./keys/secret_key.pem");
    const deploy = await contract.install(
        contractWasm,
        runtimeArguments,
        "250000000000".toString(),
        keypair.publicKey,
        "casper-test",
        [keypair]
    );
